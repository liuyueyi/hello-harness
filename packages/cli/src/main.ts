...
- 程序里可使用注入的五个能力：glob(pattern)（按前缀目录与后缀集合匹配文件、返回相对 workspace 的路径，自动跳过 node_modules/.git/.sessions 等）、read(path)（读取 workspace 内文件内容）、require(id)（加载 Node 内建模块或 workspace 内模块）、cwd()（workspace 根目录，拼绝对路径用）与 print(内容)（输出最终结论，只有 print 出的内容会进入下一轮上下文）；
...
- 只有确实需要写文件、编辑文件或执行系统命令时，才改用 write / edit / bash（这些不能写进程序）；code 工具会触发权限确认。`;
