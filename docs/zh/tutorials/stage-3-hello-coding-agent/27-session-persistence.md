...
所以这一章，我们要把 Session 写进磁盘——`.sessions/session-id.json`。接下来就进入正题。

<!-- more -->

## 一、上一版存在什么问题？

一般来讲，一个能正经聊天的 Agent，最怕的不是它答不对，而是「聊完就失忆」。回看一下 [ch26](./26-multi-turn-session) 的 Session，遗留的问题其实挺明显：
...
> 一句话：**上一版的 Session 是「过目即忘」的——它有身份、有内容，但没留下任何足迹。它不是一场「对话」，只是一场「短暂的会话」。** 简单的说，就是个用完即弃的临时工 😂

## 二、本篇解决什么问题？

那么问题来了：既然上面对话这么脆弱，我们该怎么让它「活」下来？接下来看下这一章的具体解决姿势：
...
4. **能列出来**：`list()` 返回全部历史会话（按保存时间倒序）——**小伙伴可以知道自己有哪些对话可续**；
...
- **能看清有什么**：`list()` 按时间倒序列出全部会话——**小伙伴对「我有哪些历史对话」心里有数**；
- **持久化也有边界**：`.sessions/` 落在 workspace 根内，`id` 有格式校验——**既不越界，也不怕路径注入**。

> 一句话收个尾（**最基本的**，手动加强语气）：遗留的「Session 易失、无索引、无落盘路」问题被这一章的 SessionStore 解决掉，换来的则是「一场对话 = 一个 `.sessions/` 下的 JSON 文件」——下一章就把文件读回来续跑。
...
**关键设计**：**Session 负责「对话是什么」，SessionStore 负责「对话存哪里」。** Session 不碰文件系统，Store 不碰对话逻辑——**分工明确，各自可测**。而且落盘一律经 `Workspace.resolve()`，**持久化的路径也受 workspace 边界保护**（**请注意**，这一道边界不是可有可无的装饰，是安全约定）。
...
三个设计要点（**重点关注**这三点，是这一章的精华）：

1. **一对话一文件**：文件名就是 `id`——**`.sessions/<id>.json` 和一场对话一一对应**，`load(id)` 就是按文件名取文件；
2. **id 有格式校验**：落盘前先正则校验 `id`（只允许十六进制 + 连字符）——**从根上防掉路径注入**（`../` 之类进不了文件名）；
3. **记录带时间戳**：每条记录额外保存 `savedAt`——**`list()` 按它倒序，小伙伴永远先看到最近的对话**。

> **为什么用 JSON？** 因为 `context.messages` 全是「可 JSON 化的普通对象」（role / content / toolCalls / toolCallId）——**snapshot() 产出的东西天然就是 JSON**，`JSON.stringify` 进文件、`JSON.parse` 回来，零转换成本。**最简单的格式配最简单的数据，这就是「最小实现」。** 骚操作没有，朴素的道理一条。

## 六、实现代码

### `src/session/store.ts`：会话存储

下面给出完整的 `SessionStore` 实现，注释里把关键的边界校验都标出来了：
...
**重点**：`store.save(session.snapshot())` 放在**每轮 `turn()` 之后**——**聊一句、存一次**。中途 Ctrl+C、断电、崩溃，最多丢当前这一轮，之前的对话全在磁盘上。这就是这一章最实在的使用姿势：不刻意，但稳。
...
Hello Harness v1.0 · 流式多轮对话（输入 exit 退出，Ctrl+C 取消本轮）
Session : 7898ee33-de63-4dd6-b24d-d6193c241d70
Sessions: D:\Workspace\hui\project\hello-harness\examples\stage-3\25-cli/.sessions
...
7898ee33-de63-4dd6-b24d-d6193c241d70.json
...
id: 7898ee33-de63-4dd6-b24d-d6193c241d70
messages: 44
savedAt: 1786872044291
```

![image.png](https://imgbed.ppai.top/file/1786872262343_image.png)
...
| 文件名 | 就是 Session id | `7898ee33-de63-4dd6-b24d-d6193c241d70.json` ✓ |
| 内容 | 完整 44 条消息（system/user/assistant/tool） | 消息按顺序一字不差 ✓ |
| 保存时机 | 每轮结束自动存 | 退出后文件已存在 ✓ |
| 边界 | 不越出 workspace | `dirName` 经 `workspace.resolve()` 校验 ✓ |

> 这一章同样不做无模型 demo：**SessionStore 的价值必须由真实对话来填充**——让模型真修一轮 bug，你才能在磁盘上看到「一场对话的完整脚印」，而不是一个空壳 JSON。然后就可以愉快的观察落盘结果了。
...
2. **没有会话列表入口**：`list()` 实现了，但**小伙伴没有一条命令看到历史会话**——「有什么可续」还只能靠翻目录；
...
上面这些就是 Session 持久化的基本使用姿势了，有啥用、怎么接着玩，留在下一篇 `--resume` 里逐一展开。

尽信书则不如，以上内容纯属一家之言，因个人能力有限，难免有疏漏和错误之处，如发现 bug 或者有更好的建议，欢迎批评指正，不吝感激 🙃 欢迎点赞、关注公众号「一灰灰Blog」 我们下章见
