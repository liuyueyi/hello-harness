...
到上一章为止，Coding Agent 手里有「三件套」：`read` 能看、`write` 能建、`edit` 能改。可兄弟们，这三件套全是**静态文件操作**，一碰到真实开发就集体失灵：

1. **改完代码，怎么知道改对了？**——`read` 只能把文件读回来，**它跑不了 `npm test`、编译不了 TypeScript、执行不了任何命令**。「修复这个 bug」的闭环在「验证」这一步永远断着；
2. **看不了项目结构**——`read` 只能读单个文件，**它读不了目录**。Agent 想知道「项目里有哪些文件、目录长什么样」，没有任何工具能回答；
...
> 一句话：**read / write / edit 让 Agent 能「看」和「改」，但改完没地方「验」——修 bug 的闭环缺了最关键的一环：执行。**
...
对它说：「请检查 `src/math.js` 里 `sum` 函数实现是否正确。先用 bash 运行 `node src/math.js` 验证，然后修复，最后再运行一次确认」：

```text
你 > 请检查 src/math.js 里 sum 函数实现是否正确。先用 bash 运行 node src/math.js 验证，然后修复，最后再运行一次确认。
[run:start ] Run ID : 0c7e9ca7-f209-497c-b74f-a5d2b0a79cac
[run:start ] Input  : 请检查 src/math.js 里 sum 函数实现是否正确。先用 bash 运行 node src/math.js 验证，然后修复，最后再运行一次确认。
[model:start] 思考中 …

[model:end ] 调用工具：read, bash · 976 in / 135 out · 3396ms
Step 1 · model  → 调用工具：read, bash
[tool:start] read({"path":"src/math.js"})
[tool:end  ] → "export function sum(a, b) {\n  return a - b;\n}\n\nconsole.log(\"sum(2, 3) =\", sum(2, 3));\n" · 4ms
Step 2 · tool   → read({"path":"src/math.js"}) = "export function sum(a, b) {\n  return a - b;\n}\n\nconsole.log(\"sum(2, 3) =\", sum(2, 3));\n"
[tool:start] bash({"command":"node src/math.js"})
[tool:end  ] → {"command":"node src/math.js","cwd":"C:\\Users\\bangz\\AppData\\Local\\Temp\\opencode\\hh-22-chat","stdout":"sum(2, 3) = -1\n","stderr":"","exitCode":0,"timedOut":false} · 185ms
Step 3 · tool   → bash({"command":"node src/math.js"}) = {"command":"node src/math.js","cwd":"C:\\Users\\bangz\\AppData\\Local\\Temp\\opencode\\hh-22-chat","stdout":"sum(2, 3) = -1\n","stderr":"","exitCode":0,"timedOut":false}
[model:start] 思考中 …
确实有问题——`sum` 函数写的是 `a - b`（减法），应该是 `a + b`（加法）。现在修复：


[model:end ] 调用工具：edit · 1204 in / 173 out · 3334ms
Step 4 · model  → 调用工具：edit
[tool:start] edit({"path":"src/math.js","oldString":"return a - b;","newString":"return a + b;"})
[tool:end  ] → "已替换 1 处：return a - b; → return a + b;（src/math.js）" · 4ms
Step 5 · tool   → edit({"path":"src/math.js","oldString":"return a - b;","newString":"return a + b;"}) = "已替换 1 处：return a - b; → return a + b;（src/math.js）"
[model:start] 思考中 …

[model:end ] 调用工具：bash · 1350 in / 58 out · 1600ms
Step 6 · model  → 调用工具：bash
[tool:start] bash({"command":"node src/math.js"})
[tool:end  ] → {"command":"node src/math.js","cwd":"C:\\Users\\bangz\\AppData\\Local\\Temp\\opencode\\hh-22-chat","stdout":"sum(2, 3) = 5\n","stderr":"","exitCode":0,"timedOut":false} · 143ms
Step 7 · tool   → bash({"command":"node src/math.js"}) = {"command":"node src/math.js","cwd":"C:\\Users\\bangz\\AppData\\Local\\Temp\\opencode\\hh-22-chat","stdout":"sum(2, 3) = 5\n","stderr":"","exitCode":0,"timedOut":false}
[model:start] 思考中 …
✅ 已确认并修复完成！

**问题**：`sum` 函数中写的是 `return a - b`（减法），导致 `sum(2, 3)` 输出 `-1`。

**修复**：将 `-` 改为 `+`，即 `return a + b`。

**最终验证**：`node src/math.js` 输出 `sum(2, 3) = 5`，符合预期。
[model:end ] 完成回答 · 1484 in / 110 out · 1884ms
Step 7 · model  → 完成回答
Step 8 · finish → finished
[run:end   ] completed (finished) · 10554ms
Answer  : ✅ 已确认并修复完成！
...
**最终验证**：`node src/math.js` 输出 `sum(2, 3) = 5`，符合预期。
Steps   : 4 轮 · 9 条消息 · 9 步 · 10554ms
Tokens  : 5014 in / 476 out
Status  : completed (finished)
```

兄弟，看这条完整链路，它就是修 bug 的标准姿势——**每一步都有执行证据**：

1. **read**：先读文件，看到 `return a - b;`；
2. **bash**：直接跑 `node src/math.js`——`stdout` 返回 `sum(2, 3) = -1`，**问题当场现形**：求和函数却输出 `-1`；
3. **edit**：精准替换 `a - b` → `a + b`；
4. **bash 再跑**：`node src/math.js` 输出 `sum(2, 3) = 5`——**修复被真实执行结果确认**，模型这才放心作答。

注意第一步甚至**并行**调了 `read` 和 `bash` 两个工具（`调用工具：read, bash`）——这正是 Agent Loop 的力量：**先读后跑、边读边跑，用真实输出驱动判断**。模型最后的「已确认并修复完成」，是站在 `exitCode: 0` + `stdout: sum(2, 3) = 5` 这两条铁证之上的。
...
```mermaid
flowchart LR
  P[bash command] --> V[参数校验<br/>command 非空字符串]
  V -- "否" --> PE[参数错误<br/>tool]
  V -- "是" --> S[spawn command<br/>cwd = workspace root]
  S --> T{超时?}
  T -- "是" --> K[kill 强制终止<br/>timedOut = true]
  T -- "否" --> C[close 收尾]
  K --> R[组装结果]
  C --> R[组装结果<br/>stdout / stderr / exitCode]
  R --> OK[ok: true 返回]
```

老架构和新架构，兄弟可以对照着看——从「静态三件套」到「执行通道」，Coding Agent 的能力面发生了质变：
...
3. **克制边界**：**没有做命令白名单 / 黑名单**（「允许哪些命令」是 Permission Gate 的职责，第 37 章）、**没有做 `bash` 之外的更细粒度 shell 抽象**（`git` / `npm` 等专属工具是后续演进）、**没有做交互式进程**（一次命令一个进程，`spawn` 完就等它结束）——一个能工作的最小 bash，先保证「跑得起、看得到、停得下」。
...
输出就是第三节第二屏：模型**自动**走出「read → bash 验证 → edit 修复 → bash 再验」的完整闭环——**每一步判断都建立在真实执行结果之上**。
...
双手都能跑了，可兄弟们，这正是整个系列第一次踩进「真实世界」，问题也跟着成片地冒出来：
...
**下一章：Workspace**——工具越来越多了，可兄弟们，环境越来越散：
