...
def main():
    ...
    print(f"21、34、55 的平均值为 {average:.2f}")
    return result
...
![image.png](https://imgbed.ppai.top/file/1787641004165_image.png)
...
接下来是python解释器的完整实现 `@code-runtime/src/python.ts`
...
我们在`code-chat.ts`中，提供了 `CodeAsAction` 的能力集成，那么大模型如何知道是要生成一段代码来执行相关动作的呢？ 核心就在于提示词，我们这里新装了一个Python的执行引擎，所以对应的系统提示词也需要相应的改造，一个简单的实现方式如下
...
// @code-runtime/index.ts 文件中，实现按语言的Runtime返回
...
![image.png](https://imgbed.ppai.top/file/1787641334213_image.png)

观察回写、会话隔离（`.code-sessions/`）、`withGuard` 守卫、`--code-runtime` 必须配合 `--chat/--resume` 等边界，都与上一章的 TypeScript 模式完全一致；差别只在「底下跑的是哪台发动机」。

这正是 `CodeRuntime` 契约想要的效果——**换引擎不换循环**。
