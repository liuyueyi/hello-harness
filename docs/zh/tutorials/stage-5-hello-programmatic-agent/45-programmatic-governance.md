...
一般来讲，加一层治理改一处就够。这一章动了 `core` 与 `coding` 两层，各加一块、改一处。

| 文件 | 变化 |
| --- | --- |
| `packages/core/src/runtime/scope.ts` | 新增：`RuntimeScope`（events + hooks + runId）活性作用域 |
| `packages/core/src/tool/registry.ts` | 改：`execute(call, scope?)` 成为「事件 + Hook + 执行」统一出口 |
| `packages/core/src/runtime/runtime.ts` | 改：工具事件与 Hook 委托给 `registry.execute`，不再自己发 |
| `packages/coding/src/programmatic/binding.ts` | 改：三道治理闸（清单 / require 白名单 / 预算 + 终止）+ 内层事件 |
| `packages/coding/src/tools/code.ts` | 改：`CodeActionOptions`（programCapabilities / requireAllowlist / 预算） |
...
├── core/src/
│   ├── runtime/
│   │   ├── scope.ts              ← 新增：RuntimeScope（events + hooks + runId 活性作用域）
│   │   └── runtime.ts            ← 重构：工具事件与 Hook 委托 registry.execute；
│   │                               执行前 set / 执行后 clear 活性作用域
│   ├── tool/
...
所以同样是「拿不到能力」，成因完全不一样。清单外但已经注入的 `edit`，抛的是治理层的拒绝；注册表里有但没注入的 `calculator`，程序里压根没这个名字，那个 `ReferenceError` 是 JavaScript 引擎给的。两条路子的报错来源都不是一回事，混在一起看就容易把排查方向带偏。

另外三个注入名字其实不归这张清单管。`print` 和 `cwd` 是同步的局部函数，压根不过 binding；`require` 走的是自己的白名单门，也不加计数器。清单管的是「有副作用、要走权限门」的那五个能力，剩下的不占治理预算。

这么划也有麻烦，维护分成了两处。要回答「程序能不能用 X」，得同时看代码里的注入面和注册处的清单，两处不同步就会出现「注入了却调不动」的静默失效。往长远看，注入面和清单应该合并到同一个来源，一张 SKILL.md 的 manifest 干的就是这个活——那是 ch46 的事。

记法是三句话。**清单给「程序的世界」划界，注入给「程序的世界」造物，注册表给「Harness 的世界」存货。** 收权改清单，加能力注册工具。

【截图：注入面、能力清单、注册表三个集合的交集关系；风格：three-layer；截图目标：区分「清单拒」与「ReferenceError」两种拿不到能力的路径；关键词：注入面、能力清单、注册表、交集】

### require 白名单：名字进清单才算数

`require` 是注入面里唯一能直接打开文件系统大门的缝隙。ch43 留下的问题是「程序可以 `require` 任何东西」，这一章把它收成了一张白名单。
...
归一化只剥 `node:` 这个前缀，所以 `require("node:fs")` 和 `require("fs")` 一样被拒，前缀剥不掉 `fs` 本身。程序里那个 `require` 就是在调完 `binding.assertRequireAllowed(id)` 之后才去加载模块的，位置在 `code.ts:66-69`。

一般来讲，程序想读个文件，`require("fs")` 一行的事。这里不让，得走 `read` 能力，它带 workspace 边界检查和 8000 字符截断，比裸 `fs` 安全。能开门的模块收进白名单，能读文件的路径交给带边界的能力，这就是这一章对 ch43 那个「`require` 野能力」的治理思路。
...
【截图：require 白名单放行与拒绝的对照；风格：skill-card；截图目标：说明 fs 被拒后读文件应该走 read 能力；关键词：require 白名单、node 前缀、read 能力、workspace 边界】
...
代价也得提一句。一段程序里几十次能力调用，会给 CLI 刷出几十行事件，观测统一了，噪音也统一了。加上事件 payload 里没有 `parent` 或 `depth` 字段，现在要区分内外层只能靠 `ToolCall.id` 的前缀 `program-`，想做「折叠成一张程序执行摘要」的展示层，得先补这个字段。

【截图：外层 code 与内层 bash 事件的嵌套时序；风格：swimlane；截图目标：证明内层调用复用同一 runId 与事件流；关键词：tool:start、tool:end、runId、嵌套】
...
这里有一条限制必须说清。活性作用域用的是**模块级变量**，不是 `AsyncLocalStorage`。现在的实现是单线程串行执行工具调用，`set` 和 `await` 之间不会有另一次调用插进来，所以够用。一旦要支持并发的工具调用，模块级变量就会互相踩踏，那时候才需要换成 `AsyncLocalStorage`。
...
`runtime.ts` 里原来那四行 `tool:start / beforeTool / afterTool / tool:end` 删掉了，改成在 `registry.execute(call, scope)` 周围设置活性作用域。
...
【截图：ch45 五个改动文件的职责分工；风格：skill-card；截图目标：一张卡片交代 core 加作用域、coding 加三道闸的分工；关键词：RuntimeScope、registry.execute、binding、CodeActionOptions】
...
```bash
pnpm hello --auto-approve --tools "读出 src/utils.ts 后，写一段程序：先尝试用 fs、尝试 edit，再正常组合 glob+read 找文件，观察哪几步被治理拒绝"
```
...
【截图：八项验证点对照清单；风格：checklist-card；截图目标：给出读者照做后逐条核对的验收项；关键词：能力清单、预算截断、终止开关、内层事件】
...
2. **require 野门 → 白名单门**。`fs/child_process/http` 进不来程序执行面，要读文件走带边界的 `read` 能力。ch43 那个「绕 workspace 边界的缝隙」第一次被结构性堵住；
...
【截图：ch44 三笔债的逐条对账结果；风格：checklist-card；截图目标：用对账形式交代这一章还清了哪些债；关键词：有界黑盒、require 白名单、事件统一、配置即治理】

## 九、它又引入了什么问题？

治理落地了，新账也记下几笔，接下来按严重程度排。
...
- **并发下活性作用域会踩踏**。模块级变量撑不住并行的工具调用，换成 `AsyncLocalStorage` 才安全；
...
- **`tool:end` 与 `step(tool)` 仍然重复**。运行时在发 `tool:end` 之外还发了一次 `step` 事件，两者信息重叠，这是 ch15 就记下的老问题，这一章没动它。

六笔债里最硬的是第一笔。其余五笔都是配置或者展示层的问题，唯独同步死循环是能力缺口——预算、终止、超时全都建在「事件循环还会让出来」这个前提上，前提一破整套闸就成了摆设。

【截图：ch45 治理的六笔遗留债；风格：checklist-card；截图目标：按严重程度列出同步死循环、并发作用域、预算粒度等未解决问题；关键词：同步死循环、AsyncLocalStorage、预算粒度、审计】

## 十、下一章

程序现在被三道闸围着，能碰的世界写在清单里。但清单是写在 `CodeActionOptions` 里的配置，散在注册处，它还不算一个「有名字」的治理单元。

ch44 结尾其实还留了第四笔债，比清单更根本。**单次能力粒度仍然等同一次工具调用**，「把多次已知能力组合成一个可复用单元」还没有归属。那是 ch46 要补的位置。

> **46 章把它变成一件有名字、可复用、自带清单的东西——Executable Skill。**
...
【截图：Executable Skill 的目录结构；风格：skill-card；截图目标：预告 SKILL.md 加实现的下一章形态；关键词：SKILL.md、skill.mjs、Executable Skill、治理清单】
